...
### Example 1: Scroll-Linked Typography Animation

Fade text in/out based on scroll progress using Framer Motion:
...
export const HeroText = () => {
  ...
  // Fade out between 10% and 22% scroll depth
  const opacity = useTransform(scrollYProgress, [0, 0.1, 0.22], [1, 1, 0]);
  const y = useTransform(scrollYProgress, [0, 0.22], [0, -100]);

  return (
    <motion.div style={{ opacity, y }}>
      <h1 className="text-7xl font-extralight text-white">
        Ashutosh <span className="font-normal text-transparent bg-clip-text bg-gradient-to-r from-white to-white/30">Singare</span>
      </h1>
      <p className="text-2xl font-light text-white/60">Creative Developer.</p>
    </motion.div>
  );
};
```

### Example 2: Instant Email Clipboard Copy

Allow recruiters to copy your email with a single click:

```tsx
const [copied, setCopied] = useState(false);
const EMAIL = "ashutoshsingarehere@gmail.com";
...
### Example 3: Canvas WebGL Rendering Loop

Implement continuous particle animation in ScrollyCanvas:
...
useEffect(() => {
  ...
  const animate = () => {
    ...
    // Draw particles, update positions, etc.
